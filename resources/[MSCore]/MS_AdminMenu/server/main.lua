...
local function notify(source, message)
    if source == 0 then
        print(('[MSCore ACP] %s'):format(message))
        return
    end
...
local function isRoot(source)
    return source == 0
        ...
        or IsPlayerAceAllowed(source, 'mscore.admin')
end
...
local function supportSnapshot(source, options)
    ...
    source = tonumber(source)
    local player = options.player or (source and exports.MSCore:GetPlayer(source))
    return {
        source = source,
...
local function syncCraftingPoints(target)
    TriggerClientEvent('ms_adminmenu:client:craftingSync', target or -1, craftingSyncPayload())
end

local function itemCatalogMap()
    local items = {}
    for _, item in ipairs(exports.MSCore:GetItemCatalog()) do
...
local function worldBuilderData(source)
    if not hasPermission(source, 'world') or GetResourceState('MS_WorldBuilder') ~= 'started' then
        return nil
    end
    local success, data = pcall(function()
        return exports.MS_WorldBuilder:GetAcpData(source)
    end)
...
local function payload(source)
    ...
    return {
        ...
        items = exports.MSCore:GetItemCatalog(),
...
local function refresh(source)
    if source ~= 0 and OpenMenus[source] and isAdmin(source) then
        TriggerClientEvent('ms_adminmenu:client:refresh', source, payload(source))
    end
end
...
local function result(source, success, message)
    TriggerClientEvent('ms_adminmenu:client:result', source, {
...
local function activeTarget(data)
    ...
    local player = target and exports.MSCore:GetPlayer(target)
...
local function cleanupSupportLogs()
    ...
    MySQL.update.await(([[DELETE FROM mscore_support_logs
...
MySQL.ready(function()
    ...
    for _, row in ipairs(MySQL.query.await('SELECT * FROM mscore_crafting_recipes ORDER BY id') or {}) do
        local recipe = rowToRecipe(row)
        CraftingRecipes[recipe.id] = recipe
    end
    for _, row in ipairs(MySQL.query.await('SELECT * FROM mscore_crafting_points ORDER BY id') or {}) do
        local point = rowToPoint(row)
        CraftingPoints[point.id] = point
    end
    Ready = true
    syncCraftingPoints()
    print(('[MSCore ACP] %d Rechteprofile, %d Rezepte und %d Crafting-Punkte geladen.'):format(
...
local function setPermissions(source, data)
    ...
    MySQL.update.await([[
        INSERT INTO mscore_admin_permissions
...
local function createRecipe(source, data)
    ...
    recipe.id = MySQL.insert.await([[
        INSERT INTO mscore_crafting_recipes
...
local function deleteRecipe(source, data)
    ...
    MySQL.update.await('DELETE FROM mscore_crafting_recipes WHERE id = ?', { id })
    ...
    for _, point in pairs(CraftingPoints) do
        ...
        if changed then
            ...
            MySQL.update.await(
                'UPDATE mscore_crafting_points SET recipe_ids = ? WHERE id = ?',
...
local function createCraftingPoint(source, data)
    ...
    point.id = MySQL.insert.await([[
        INSERT INTO mscore_crafting_points
...
local function createDatabaseItem(source, data)
    ...
    local catalog = exports.MSCore:GetItemCatalog()
    ...
    local callOk, success, itemOrError = pcall(function()
        return exports.MSCore:CreateItem(data, getLicense(source) or 'console')
    end)
    if not callOk then
        print(('[MSCore ACP] Itemcreator-Fehler: %s'):format(tostring(success)))
...
local function deleteDatabaseItem(source, data)
    ...
    local callOk, success, itemOrError = pcall(function()
        return exports.MSCore:DeleteItem(name)
    end)
    if not callOk then
        print(('[MSCore ACP] Item-Löschfehler: %s'):format(tostring(success)))
...
RegisterNetEvent('ms_adminmenu:server:open', function()
    local source = source
...
RegisterNetEvent('ms_adminmenu:server:refresh', function()
    local source = source
...
RegisterNetEvent('ms_adminmenu:server:execute', function(action, data)
    local source = source
    if not Ready or not isAdmin(source) then
        OpenMenus[source] = nil
        TriggerClientEvent('ms_adminmenu:client:forceClose', source)
    ...
    if action == 'setWeather' then
        ...
        TriggerClientEvent('ms_adminmenu:client:applyWeather', -1, {
            id = weather.id,
            hash = weather.hash,
...
local function craftingView(source, point, player)
    ...
    TriggerClientEvent('ms_adminmenu:client:openCrafting', source, {
        point = { id = point.id, label = point.label },
        recipes = recipes
    })
end

RegisterNetEvent('ms_adminmenu:server:openCrafting', function(pointId)
    local source = source
...
RegisterNetEvent('ms_adminmenu:server:craft', function(pointId, recipeId)
    local source = source
    ...
    TriggerClientEvent('ms_adminmenu:client:craftingBusy', source, {
        duration = recipe.duration,
        label = recipe.label
    })
    ...
    player = exports.MSCore:GetPlayer(source)
...
local function playerSourceFromNetworkId(networkId)
    ...
    if owner and owner > 0 and GetPlayerPed(owner) == entity
        and exports.MSCore:GetPlayer(owner) then
        return owner
    end
...
AddEventHandler('weaponDamageEvent', function(sender, data)
    ...
    local actorPlayer = exports.MSCore:GetPlayer(sender)
    local targetPlayer = exports.MSCore:GetPlayer(victimSource)
...
AddEventHandler('mscore:server:playerLoaded', function(source, player)
    ...
    SetTimeout(1200, function()
        ...
        if weather then
            TriggerClientEvent('ms_adminmenu:client:applyWeather', source, {
                id = weather.id,
                hash = weather.hash,
                transition = CurrentTransition
            })
...
AddEventHandler('mscore:server:onboardingCompleted', function(source)
...
local function flushSupportLogs()
    ...
    local success, err = pcall(MySQL.insert.await, ([[
        INSERT INTO mscore_support_logs
    ...
    if not success then
        ...
        if now - LastSupportLogError >= 10000 then
            ...
            print(('[MSCore ACP] Support-Logs konnten nicht gespeichert werden: %s'):format(
                tostring(err)
            ))
        end
    elseif DroppedSupportLogs > 0 then
        print(('[MSCore ACP] %d Support-Logs wurden wegen voller Warteschlange verworfen.'):format(
            DroppedSupportLogs
        ))
