...
local function notify(source, message)
    if source == 0 then
        print(('[MSCore World Builder] %s'):format(message))
        return
    end
...
local function isBuilder(source)
    ...
    local success, allowed = pcall(function()
        return exports.MS_AdminMenu:HasPermission(source, 'world')
    end)
    return success and allowed == true
end

local function audit(source, action, detail)
    print(('[MSCore World Builder] %s (%d) | %s | %s'):format(
...
local function syncAll()
    TriggerClientEvent('ms_worldbuilder:client:sync', -1, definitions())
end

local function refreshBuilder(source)
    if isBuilder(source) then
        TriggerClientEvent('ms_worldbuilder:client:builderData', source, builderPayload())
        TriggerClientEvent('ms_adminmenu:client:worldBuilderData', source, acpPayload())
    end
end
...
local function result(source, success, message)
    ...
    TriggerClientEvent('ms_adminmenu:client:externalResult', source, {
        success = success == true,
        message = message
    })
...
RegisterNetEvent('ms_worldbuilder:server:create', function(kind, data)
    local source = source
...
RegisterNetEvent('ms_worldbuilder:server:delete', function(kind, id)
    local source = source
    ...
    local definitionsByKind = {
        ...
        storage = { items = Storages, tableName = 'mscore_storages' },
        door = { items = Doors, tableName = 'mscore_doors' }
    }
...
local function loadInventory(storageId, ownerKey)
    local encoded = MySQL.scalar.await(
        'SELECT items FROM mscore_storage_inventories WHERE storage_id = ? AND owner_key = ?',
        { storageId, ownerKey }
    )
    if not encoded then
        MySQL.update.await(
            'INSERT IGNORE INTO mscore_storage_inventories (storage_id, owner_key, items) VALUES (?, ?, ?)',
            { storageId, ownerKey, '{}' }
        )
        return {}
    end
...
local function catalogMap()
    local items = {}
    for _, item in ipairs(exports.MSCore:GetItemCatalog()) do items[item.name] = item end
    return items
end
...
local function storageView(source, storage, player, ownerKey, inventory)
    ...
    TriggerClientEvent('ms_worldbuilder:client:openStorage', source, {
        id = storage.id,
        label = storage.label,
        type = storage.type,
...
RegisterNetEvent('ms_worldbuilder:server:transfer', function(storageId, direction, itemName, rawAmount)
    ...
    if not storage or not player or (direction ~= 'deposit' and direction ~= 'withdraw')
        ...
        return TriggerClientEvent('ms_worldbuilder:client:result', source, {
            success = false,
...
RegisterNetEvent('ms_worldbuilder:server:interactDoor', function(doorId)
    toggleDoor(source, doorId, true)
end)

RegisterNetEvent('ms_worldbuilder:server:builderToggleDoor', function(doorId)
    local source = source
    if not isBuilder(source) then return end
