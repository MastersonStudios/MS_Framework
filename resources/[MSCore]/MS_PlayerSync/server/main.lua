...
local function debugLog(message, ...)
    ...
    print(('[MS_PlayerSync] ' .. message):format(...))
end
...
local function upsertPlayer(playerSource, player, force)
    ...
    TriggerClientEvent('ms_playersync:client:upsert', -1, publicState, Revision)
...
local function removePlayer(playerSource)
    ...
    TriggerClientEvent('ms_playersync:client:remove', -1, playerSource, Revision)
...
local function sendSnapshot(target)
    TriggerClientEvent('ms_playersync:client:snapshot', target, getSnapshot(), Revision)
end

RegisterNetEvent('ms_playersync:server:requestSnapshot', function()
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if configuredSlots > PlayerSyncConfig.MaxPlayers then
        print(('[MS_PlayerSync] WARNING: sv_maxclients is %d, but this resource is configured for %d players.'):format(
            configuredSlots,
    ...
    if GetConvar('sv_stateBagStrictMode', 'false') ~= 'true' then
        print('[MS_PlayerSync] WARNING: Set sv_stateBagStrictMode true to keep player state server-authoritative.')
    end
...
CreateThread(function()
    while true do
        ...
        local corePlayers = exports.MSCore:GetPlayers()
