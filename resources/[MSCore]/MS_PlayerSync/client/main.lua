...
local function upsertState(state, revision)
    ...
    TriggerEvent('ms_playersync:client:updated', state.source, state)
end

RegisterNetEvent('ms_playersync:client:snapshot', function(snapshot, revision)
    ...
    TriggerEvent('ms_playersync:client:snapshotUpdated', Revision)
end)

RegisterNetEvent('ms_playersync:client:upsert', upsertState)

RegisterNetEvent('ms_playersync:client:remove', function(playerSource, revision)
    ...
    TriggerEvent('ms_playersync:client:removed', playerSource)
end)
...
AddStateBagChangeHandler(PlayerSyncConfig.StateBagKey, nil, function(bagName, _, value)
    ...
    if value == false then
        if SyncedPlayers[bagPlayerSource] then
            ...
            TriggerEvent('ms_playersync:client:removed', bagPlayerSource)
        end
        return
    end
...
CreateThread(function()
    ...
    TriggerServerEvent('ms_playersync:server:requestSnapshot')
end)
