...
RegisterNetEvent('ms_mapeditor:client:sync', function(objects)
...
RegisterNetEvent('ms_mapeditor:client:upsert', function(object)
...
RegisterNetEvent('ms_mapeditor:client:remove', function(id)
    id = tonumber(id)
    deleteEntityFor(id)
    Objects[id] = nil
...
RegisterNetEvent('ms_mapeditor:client:create', function(model)
    model = model or MapEditorConfig.ObjectCatalog[1].model
    startEditor(model:lower())
end)

RegisterNetEvent('ms_mapeditor:client:edit', function(requestedId)
...
RegisterNetEvent('ms_mapeditor:client:deleteNearest', function(requestedId)
    if Editor then
        local id = Editor.id
        if not id then return notify('Ein noch nicht gespeichertes Objekt kann nur abgebrochen werden.') end
        cleanupEditor(false)
        TriggerServerEvent('ms_mapeditor:server:delete', id)
        return
    end
...
RegisterNetEvent('ms_mapeditor:client:listNearby', function()
    local playerCoords = GetEntityCoords(PlayerPedId())
    local nearby = {}
    for id, object in pairs(Objects) do
        local dx, dy, dz = object.x - playerCoords.x, object.y - playerCoords.y, object.z - playerCoords.z
...
bindHold('mscore_map_forward', 'Mapeditor: vorwärts', 'W', 'forward')
bindHold('mscore_map_backward', 'Mapeditor: rückwärts', 'S', 'backward')
bindHold('mscore_map_left', 'Mapeditor: links', 'A', 'left')
bindHold('mscore_map_right', 'Mapeditor: rechts', 'D', 'right')
bindHold('mscore_map_up', 'Mapeditor: hoch', 'PAGEUP', 'up')
bindHold('mscore_map_down', 'Mapeditor: runter', 'PAGEDOWN', 'down')
bindHold('mscore_map_rotate_left', 'Mapeditor: links drehen', 'Q', 'rotateLeft')
bindHold('mscore_map_rotate_right', 'Mapeditor: rechts drehen', 'E', 'rotateRight')
bindHold('mscore_map_pitch_forward', 'Mapeditor: vorwärts kippen', 'R', 'pitchForward')
bindHold('mscore_map_pitch_backward', 'Mapeditor: rückwärts kippen', 'F', 'pitchBackward')
bindHold('mscore_map_roll_left', 'Mapeditor: nach links kippen', 'Z', 'rollLeft')
bindHold('mscore_map_roll_right', 'Mapeditor: nach rechts kippen', 'X', 'rollRight')
bindHold('mscore_map_fast', 'Mapeditor: schnell bewegen', 'LSHIFT', 'fast')

RegisterCommand('mscore_map_confirm', function()
    if not Editor then return end
    local id, payload = Editor.id, editorPayload()
    cleanupEditor(false)
    if id then
        TriggerServerEvent('ms_mapeditor:server:update', id, payload)
    else
        TriggerServerEvent('ms_mapeditor:server:create', payload)
    end
end, false)
RegisterKeyMapping('mscore_map_confirm', 'Mapeditor: speichern', 'keyboard', 'RETURN')

RegisterCommand('mscore_map_cancel', function()
    if Editor then
        cleanupEditor(true)
        notify('Bearbeitung abgebrochen.')
    end
end, false)
RegisterKeyMapping('mscore_map_cancel', 'Mapeditor: abbrechen', 'keyboard', 'BACK')

RegisterCommand('mscore_map_ground', function()
...
RegisterKeyMapping('mscore_map_ground', 'Mapeditor: auf Boden setzen', 'keyboard', 'G')

CreateThread(function()
    TriggerServerEvent('ms_mapeditor:server:requestSync')
    while true do
        local playerCoords = GetEntityCoords(PlayerPedId())
        for id, object in pairs(Objects) do
            local dx, dy, dz = object.x - playerCoords.x, object.y - playerCoords.y, object.z - playerCoords.z
