...
local function spawnNpc(id, npc)
    ...
    CreateThread(function()
        ...
        if not hash or not Definitions.npcs[id] or NpcEntities[id] then
            NpcLoading[id] = nil
            NpcFailures[id] = true
            return print(('[Frontier World Builder] NPC-Modell "%s" konnte nicht geladen werden.'):format(npc.model))
        end
...
RegisterCommand(WorldBuilderConfig.Command, function()
    if BuilderOpen then
        ...
        TriggerServerEvent('frontier_worldbuilder:server:openBuilder')
    end
end, false)

RegisterKeyMapping(
    WorldBuilderConfig.Command,
    'Frontier World Builder öffnen',
    'keyboard',
    WorldBuilderConfig.DefaultKey
)

RegisterNetEvent('frontier_worldbuilder:client:sync', function(data)
...
RegisterNetEvent('frontier_worldbuilder:client:updateDoor', function(door)
    local id = tonumber(door and door.id)
    if not id then return end
    door.id = id
    Definitions.doors[id] = door
    applyDoor(door)
    if BuilderOpen then TriggerServerEvent('frontier_worldbuilder:server:openBuilder') end
end)

RegisterNetEvent('frontier_worldbuilder:client:openBuilder', function(data)
    BuilderOpen = true
    StorageOpen = false
...
RegisterNetEvent('frontier_worldbuilder:client:builderData', function(data)
    if BuilderOpen then
        SendNUIMessage({ action = 'builderData', data = builderEnvelope(data) })
    end
end)

RegisterNetEvent('frontier_worldbuilder:client:openStorage', function(data)
    StorageOpen = true
    BuilderOpen = false
    SetNuiFocus(true, true)
...
RegisterNetEvent('frontier_worldbuilder:client:result', function(data)
    SendNUIMessage({
        action = 'result',
...
RegisterNUICallback('storageTransfer', function(data, cb)
    ...
    TriggerServerEvent(
        'frontier_worldbuilder:server:transfer',
        data.storageId,
        data.direction,
        data.item,
        data.amount
...
RegisterCommand('+frontier_world_interact', function()
    ...
    elseif NearestInteraction.kind == 'door' then
        TriggerServerEvent('frontier_worldbuilder:server:interactDoor', NearestInteraction.id)
    end
end, false)
RegisterCommand('-frontier_world_interact', function() end, false)
RegisterKeyMapping(
    '+frontier_world_interact',
...
CreateThread(function()
    ...
    Wait(1200)
    TriggerServerEvent('frontier_worldbuilder:server:requestSync')
end)
