...
local function notify(source, message)
    if source == 0 then
        print(('[Frontier Guarma] %s'):format(message))
        return
    end
...
local function sendAdminAlert(playerSource, characterName)
    ...
    for _, adminId in ipairs(GetPlayers()) do
        ...
        if adminId and IsPlayerAceAllowed(adminId, GuarmaConfig.AdminPermission) then
            TriggerClientEvent('frontier_guarma:client:adminAlert', adminId, {
                source = playerSource,
...
local function startForPlayer(source, player)
    ...
    if player.metadata.guarmaOnboardingComplete then
        if isOnGuarma(player.coords) then
            TriggerClientEvent('frontier_guarma:client:setIslandMode', source, true)
        end
        return
    end
    ...
    TriggerClientEvent('frontier_guarma:client:start', source, resume)
end
...
RegisterNetEvent('frontier_guarma:server:beachArrived', function()
    local source = source
...
RegisterNetEvent('frontier_guarma:server:complete', function()
    local source = source
    local entry = ActiveOnboardings[source]
    local player = exports.frontier_core:GetPlayer(source)
    if not entry or not player or player.characterId ~= entry.characterId then return end
    if distanceTo(source, GuarmaConfig.Port) > GuarmaConfig.CompletionRadius then
        TriggerClientEvent('frontier_guarma:client:completionRejected', source)
    ...
    TriggerClientEvent('frontier_guarma:client:completeConfirmed', source)
    notify(source, 'Tutorial abgeschlossen. Willkommen auf Guarma!')
    TriggerEvent('frontier:server:onboardingCompleted', source, player)
end)

RegisterNetEvent('frontier_guarma:server:teleportAdmin', function(locationId)
    local source = source
    if not isAdmin(source) or type(locationId) ~= 'string' then return end
    for _, location in ipairs(GuarmaConfig.AdminLocations) do
        if location.id == locationId then
            TriggerClientEvent('frontier_guarma:client:teleport', source, location.coords)
            print(('[Frontier Guarma] %s (%d) teleportiert nach %s.'):format(
...
local function openAdminMenu(source)
    ...
    TriggerClientEvent('frontier_guarma:client:openAdminMenu', source, {
...
RegisterCommand(GuarmaConfig.ResetCommand, function(source, args)
    ...
    local player = exports.frontier_core:GetPlayer(target)
    ...
    TriggerClientEvent('frontier_guarma:client:forceStop', target)
    SetTimeout(250, function()
        local currentPlayer = exports.frontier_core:GetPlayer(target)
...
AddEventHandler('frontier:server:playerUnloaded', function(source)
    ActiveOnboardings[source] = nil
    BeachNotifications[source] = nil
    TriggerClientEvent('frontier_guarma:client:forceStop', source)
end)
