...
local function inventoryPayload(playerSource)
    ...
    for itemName, amount in pairs(player:getInventory()) do
        local item = exports.frontier_core:GetItem(itemName)
    ...
    return {
        ...
        limits = exports.frontier_core:GetInventoryLimits(),
...
RegisterNetEvent('ms_inventory:server:give', function(itemName, amount, targetSource)
    ...
    runAction(playerSource, 'give', function()
        ...
        player:save()
        target:save()
        TriggerClientEvent('frontier:client:notify', targetSource, ('%dx %s von %s erhalten.'):format(
            amount,
            item.label,
...
RegisterNetEvent('ms_inventory:server:unequip', function(slotKey)
    ...
    runAction(playerSource, 'unequip', function()
        ...
        local item = itemName and exports.frontier_core:GetItem(itemName)
...
AddEventHandler('frontier:server:itemChanged', function(playerSource)
    scheduleRefresh(playerSource)
end)

AddEventHandler('frontier:server:itemCatalogUpdated', function()
