...
local function worldInteractionNearby()
    if GetResourceState('frontier_worldbuilder') ~= 'started' then return false end
    local success, nearby = pcall(function()
        return exports.frontier_worldbuilder:HasNearbyInteraction()
    end)
...
RegisterNetEvent('frontier_adminmenu:client:open', openAcp)
...
RegisterNetEvent('frontier_adminmenu:client:externalResult', function(data)
...
RegisterNetEvent('frontier_adminmenu:client:worldBuilderData', function(data)
    if MenuOpen then SendNUIMessage({ action = 'worldData', data = data }) end
end)

RegisterNetEvent('frontier_adminmenu:client:forceClose', closeUi)
...
RegisterNetEvent('frontier_adminmenu:client:craftingSync', function(points)
    CraftingPoints = {}
...
RegisterNetEvent('frontier_adminmenu:client:openCrafting', function(data)
    if MenuOpen then TriggerServerEvent('frontier_adminmenu:server:close') end
    MenuOpen = false
    CraftingOpen = true
    SetNuiFocus(true, true)
...
RegisterNetEvent('frontier_adminmenu:client:craftingBusy', function(data)
    if CraftingOpen then SendNUIMessage({ action = 'craftingBusy', data = data }) end
end)

RegisterNetEvent('frontier_adminmenu:client:craftResult', function(data)
    SendNUIMessage({
        action = 'craftResult',
        success = data and data.success == true,
...
RegisterNUICallback('refresh', function(_, cb)
    if MenuOpen then TriggerServerEvent('frontier_adminmenu:server:refresh') end
    cb({ ok = true })
end)
...
RegisterNUICallback('worldCreate', function(data, cb)
    ...
    TriggerServerEvent('frontier_worldbuilder:server:create', data.kind, data.data or {})
    cb({ ok = true })
end)

RegisterNUICallback('worldDelete', function(data, cb)
    if not MenuOpen or type(data) ~= 'table' then return cb({ ok = false }) end
    TriggerServerEvent('frontier_worldbuilder:server:delete', data.kind, data.id)
    cb({ ok = true })
end)

RegisterNUICallback('worldToggleDoor', function(data, cb)
    if not MenuOpen or type(data) ~= 'table' then return cb({ ok = false }) end
    TriggerServerEvent('frontier_worldbuilder:server:builderToggleDoor', data.id)
    cb({ ok = true })
end)
...
RegisterNUICallback('craft', function(data, cb)
    ...
    TriggerServerEvent('frontier_adminmenu:server:craft', data.pointId, data.recipeId)
    cb({ ok = true })
end)

RegisterCommand('+frontier_crafting_interact', function()
    if MenuOpen or CraftingOpen or worldInteractionNearby() or not NearestCraftingPoint then return end
    TriggerServerEvent('frontier_adminmenu:server:openCrafting', NearestCraftingPoint.id)
end, false)
RegisterCommand('-frontier_crafting_interact', function() end, false)
RegisterKeyMapping(
    '+frontier_crafting_interact',
