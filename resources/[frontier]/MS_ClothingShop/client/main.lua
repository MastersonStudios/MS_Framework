...
local function previewProduct(itemName)
    ...
    if not componentHash or type(slot) ~= 'string' then
        return TriggerEvent('frontier:client:notify', 'Für dieses Kleidungsstück ist keine Vorschau verfügbar.')
    end
...
local function openNearestShop()
    ...
    if inventoryIsOpen() then
        return TriggerEvent('frontier:client:notify', 'Schließe zuerst dein Inventar.')
    end
    if not NearestShop then
        return TriggerEvent('frontier:client:notify', 'Du bist bei keinem Bekleidungshändler.')
    end
