...
local function log(message, ...)
    print(('[frontier_core:version] ' .. message):format(...))
end
...
local function finishCheck(status, latest, errorMessage)
    ...
    TriggerEvent('frontier:server:versionChecked', copyState())
end
...
local function checkForUpdates(force)
    ...
    PerformHttpRequest(url, function(statusCode, responseBody, _, requestError)
        ...
        ['User-Agent'] = ('MS-Framework/%s'):format(CURRENT_VERSION)
    })
...
Frontier.Version = CURRENT_VERSION
Frontier.GetVersion = GetFrameworkVersion
Frontier.GetVersionState = GetFrameworkVersionState
    ...
    if action == 'check' then
        if playerSource ~= 0
            and not IsPlayerAceAllowed(
                playerSource,
                tostring(Settings.AdminAce or 'frontier.version.check')
            )
        then
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    log('MS Framework Version %s', CURRENT_VERSION)
