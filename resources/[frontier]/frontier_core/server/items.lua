...
local function seedConfigItems()
    for name, item in pairs(Config.Items or {}) do
        MySQL.update.await([[
            INSERT IGNORE INTO frontier_items
...
local function loadItems()
    Items = {}
    for _, row in ipairs(MySQL.query.await('SELECT * FROM frontier_items ORDER BY label') or {}) do
...
MySQL.ready(function()
    ...
    seedConfigItems()
    loadItems()
    print(('[Frontier Core] %d Datenbank-Items geladen.'):format((function()
        local count = 0
...
function Frontier.GetItemDefinition(name)
    return type(name) == 'string' and Items[name] or nil
end

function GetItem(name)
    return Frontier.GetItemDefinition(name)
end
...
function CreateItem(data, createdBy)
    ...
    Items[item.name] = item
    TriggerEvent('frontier:server:itemCatalogUpdated', GetItemCatalog())
    return true, item
end
...
local function itemUsage(name)
    for _, player in pairs(Frontier.GetPlayers and Frontier.GetPlayers() or {}) do
    ...
    if scalarSafe('SELECT COUNT(*) FROM frontier_characters WHERE metadata LIKE ?', { pattern }) > 0 then
        return 'Das Item befindet sich noch in einem gespeicherten Spielerinventar.'
    end
    if scalarSafe('SELECT COUNT(*) FROM frontier_storage_inventories WHERE items LIKE ?', { pattern }) > 0 then
        return 'Das Item befindet sich noch in einem Storage.'
    end
    if scalarSafe('SELECT COUNT(*) FROM frontier_crafting_recipes WHERE output_item = ? OR ingredients LIKE ?', {
        name, pattern
    }) > 0 then
...
function DeleteItem(name)
    ...
    local affected = MySQL.update.await('DELETE FROM frontier_items WHERE name = ? AND is_system = 0', { name })
